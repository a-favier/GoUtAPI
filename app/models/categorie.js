...
const categorie={
    ...
    getCategorieByEvent:(idEvent,callback) => {
        return db.query("SELECT `categorie`.`id`, `data_categorie`.`name` FROM `gout`.`categorie` LEFT JOIN `data_categorie` ON `categorie`.`id_data_categorie` = `data_categorie`.`id` WHERE `categorie`.`id_event` = ?",[idEvent],callback);
    },
